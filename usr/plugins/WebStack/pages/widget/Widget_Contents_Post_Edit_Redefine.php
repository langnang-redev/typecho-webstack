...
define('WNAVI','extending.php?panel=WebStack%2Fpages%2Fadmin%2Fwrite-navi.php&');

class Widget_Contents_Post_Edit_Redefine extends Widget_Abstract_Contents implements Widget_Interface_Do
{
    /**
     * 自定义字段的hook名称
     *
    ...
    {
        return 'page=' . $this->getPageOffset('created', $created, 'navigation', $status,
    ...
    public function execute()
    {
        ...
        if (!empty($this->request->cid) && 'delete' != $this->request->do) {
            $this->db->fetchRow($this->select()
                ->where('table.contents.type = ? OR table.contents.type = ?', 'navigation', 'post_draft')
            ...
            if (!$this->have()) {
                throw new Typecho_Widget_Exception(_t('导航链接不存在'), 404);
    ...
    {
        if ('navigation' == $value['type'] || 'page' == $value['type']) {
    ...
    public function writePost()
    {
        $contents = $this->request->from('password', 'allowComment',
            'allowPing', 'allowFeed', 'slug', 'tags', 'text', 'visibility','order');
        ...
        if ($this->request->is('do=publish')) {
            ...
            /** 设置提示信息 */
            $this->widget('Widget_Notice')->set('navigation' == $this->type ?
                _t('导航链接 "<a href="%s">%s</a>" 已经发布', $this->permalink, $this->title) :
                _t('导航链接 "%s" 等待审核', $this->title), 'success');
            ...
            /** 页面跳转 */
            $this->response->redirect(Typecho_Common::url(MNAVI . $pageQuery, $this->options->adminUrl));
        } else {
            /** 保存导航链接 */
            ...
            if ($this->request->isAjax()) {
                ...
                /** 返回原页面 */
                $this->response->redirect(Typecho_Common::url(WNAVI . $this->cid, $this->options->adminUrl));
            }
        }
    }
    ...
    public function deletePost()
    {
        ...
        foreach ($posts as $post) {
            ...
            $postObject = $this->db->fetchObject($this->db->select('status', 'type')
                ->from('table.contents')->where('cid = ? AND type = ?', $post, 'navigation'));
        ...
        /** 设置提示信息 */
        $this->widget('Widget_Notice')->set($deleteCount > 0 ? _t('导航链接已经被删除') : _t('没有导航链接被删除'),
